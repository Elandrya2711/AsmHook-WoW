...
using System;
using Tanaris.BO;
using Tanaris.Constants;
using Tanaris.Helper;
using Tanaris.Hooks;
using Tanaris.Managers;
using UserApi;
using WrapperObjects = UserApi.WoWObjects;

namespace Tanaris.WoWObjects
{
    internal class LocalPlayer : Player
    {
        ...
        public void TeleportToCords(Vector3 location, float travelZ)
        {
            try
            {
                ...
                {
                    if ((CalcDistance(WoW.LocalPlayer.Location.X, location.X) > 50)
                     || (CalcDistance(WoW.LocalPlayer.Location.Y, location.Y) > 50))
                    {
                        float nx, ny;
                        if ((CalcDistance(WoW.LocalPlayer.Location.X, location.X) > 50))
                        {
                            if (location.X > WoW.LocalPlayer.Location.X)
                                nx = (WoW.LocalPlayer.Location.X + 50);
                            else
                                nx = (WoW.LocalPlayer.Location.X - 50);
                        }
                        else
                        ...
                        if ((CalcDistance(WoW.LocalPlayer.Location.Y, location.Y) > 50))
                        {
                            if (location.Y > WoW.LocalPlayer.Location.Y)
                                ny = (WoW.LocalPlayer.Location.Y + 50);
                            else 
                                ny = (WoW.LocalPlayer.Location.Y - 50);
                        }
                        else
                        ...
                        _wow.WriteFloat(WoW.Playerbase + 0x7A0, travelZ);
